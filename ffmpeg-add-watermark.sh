...
case $ans in
  ...
  *)  echo "WARNING: invalid option selected, defaulting to bottom-left position."
  ...
  ffmpeg -y $start_opt $end_opt -i "$1" -loop 1 -i "Watermark.png" \
  -movflags +faststart \
  $preset \
  -filter_complex \
  "color=black:16x16:d=$total[base];
  [0:v]fade=out:st=$fadetime:d=$fadeduration,setpts=PTS-STARTPTS[v0];
  $wmstream1
  [base][v0]scale2ref[base][v0];
  [base][v0]overlay,setsar=1[tmp];
  $wmstream2
  $wmstream3
  [0:a]afade=out:st=$fadetime:d=$fadeduration[0a];
  [0a]concat=n=1:v=0:a=1[outa]" \
